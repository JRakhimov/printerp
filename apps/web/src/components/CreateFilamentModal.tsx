...
export const CreateFilamentModal: React.FC<CreateFilamentModalProps> = ({ isOpen, onClose }) => {
  ...
  const onSubmit = async (data: CreateFilamentDto) => {
    try {
      await createFilament.mutateAsync(data);
      reset();
      onClose();
    } catch (err) {
      ...
      <div className="bg-slate-900 border border-slate-800 w-full max-w-md rounded-2xl p-5 shadow-2xl space-y-4 mb-8">
        <div className="flex items-center justify-between border-b border-slate-800 pb-3">
          <h3 className="text-sm font-bold text-white flex items-center gap-2">
            ...
            Добавить новую катушку филамента
          </h3>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-3">
          ...
          <div className="flex items-center justify-end space-x-2 pt-2 border-t border-slate-800">
            ...
            >
              {createFilament.isPending && <Loader2 className="w-3.5 h-3.5 animate-spin" />}
              <span>Сохранить филамент</span>
            </button>
          </div>
        </form>
      </div>
